...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 24-03-2026 a las 13:57:23
...
--
DELIMITER $$
CREATE TRIGGER `trg_delete_autor` AFTER DELETE ON `autor` FOR EACH ROW INSERT INTO audi_socio(audi_fechaModificacion, audi_accion, socNombre_anterior)
VALUES (NOW(), 'DELETE AUTOR', OLD.aut_apellido)
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trg_update_autor` AFTER UPDATE ON `autor` FOR EACH ROW INSERT INTO audi_socio(audi_fechaModificacion, audi_accion, socNombre_nuevo)
VALUES (NOW(), 'UPDATE AUTOR', NEW.aut_apellido)
$$
DELIMITER ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista1`  AS SELECT `s`.`soc_nombre` AS `soc_nombre`, `l`.`lib_titulo` AS `lib_titulo` FROM ((`socio` `s` join `prestamo` `p`) join `libro` `l`) WHERE `s`.`soc_numero` = `p`.`soc_copiaNumero` AND `l`.`lib_isbn` = `p`.`lib_copiaISBN` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista2`  AS SELECT `l`.`lib_titulo` AS `lib_titulo`, `a`.`aut_apellido` AS `aut_apellido` FROM ((`libro` `l` join `tipo_autores` `t`) join `autor` `a`) WHERE `l`.`lib_isbn` = `t`.`copiaISBN` AND `a`.`aut_codigo` = `t`.`copiaAutor` ;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `eliminar_prestamos` ON SCHEDULE EVERY 1 DAY STARTS '2026-01-01 00:00:00' ENDS '2026-12-31 23:59:59' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM prestamo
